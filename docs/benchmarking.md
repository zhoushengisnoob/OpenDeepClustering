...
Reference runs use `dec_mnist_reference.yaml` and `idec_mnist_reference.yaml`. MNIST is not downloaded implicitly; place it under the configured root or set `download: true` deliberately. Each output contains the resolved experiment configuration, Git commit, Python/package/CUDA/GPU environment, per-seed metrics and aggregate mean/std.
